...
Note that if you want to reference any classes that your plugin doesn't provide, you should correctly reference that
in the xref:mod-information.adoc#_dependencies[dependencies] section of the `mod.json`.

While Andraste currently doesn't isolate the classpath, meaning you could load any DLL of the users current mods,
explicitly stating dependencies helps for maximum compatibility and transparency to the user.

NOTE: This even includes specifying dependencies on Andraste Features and Versions as well as the game
